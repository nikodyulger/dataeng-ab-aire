...
def apply_rename(df: pl.DataFrame, object_key: str) -> pl.DataFrame:
    ...
    config = load_config()
    target_mapping = (
        config["RENAME_MAPS"]["CONTAMINANTE"]
        if "CONTAMINANTE" in object_key.upper()
        else config["RENAME_MAPS"]["METEO"]
    )

    # Filtrar solo las columnas que existen en el DataFrame para mapping exacto
    target_mapping_filtered = {
        old_name: new_name
        for old_name, new_name in target_mapping.items()
        if old_name in df.columns
    }

    missing_columns = set(target_mapping.keys()) - set(df.columns)
    if missing_columns:
        logger.warning(
            f"Columnas esperadas no encontradas en el DataFrame: {missing_columns}. "
            "Buscando coincidencias parciales..."
        )

    # Aplicar mapping exacto
    if target_mapping_filtered:
        df = df.rename(target_mapping_filtered)
        logger.info(
            f"Renombradas {len(target_mapping_filtered)} columnas por mapping exacto"
        )

    # Buscar coincidencias parciales para columnas faltantes
    pattern_mappings = {
        "PRB": "pres",
    }
    ...
    for col in df.columns:
        for pattern, new_name in pattern_mappings.items():
            if (
                pattern.upper() in col.upper()
                and col not in target_mapping_filtered.values()
            ):
                additional_renames[col] = new_name
                logger.info(
                    f"Columna '{col}' coincide con patrón '{pattern}', renombrando a '{new_name}'"
                )
                break  # Solo el primer patrón que coincida

    if additional_renames:
        df = df.rename(additional_renames)
        logger.info(f"Renombradas {len(additional_renames)} columnas por patrones")
...
def impute_nulls(df: pl.DataFrame) -> pl.DataFrame:
    ...
    df = df.filter(
        ~pl.all_horizontal(cs.numeric().is_null())
    )  # filtramos filas con todos los parametros nulos -> suele ser la primera fila
    df = df.fill_null(
        strategy="forward"
    )  # rellena huecos con el valor anterior, es un técnica común para timeseries
    ...
    return (
        df.with_columns(
            [
                ...
                (
                    (A * pl.col("temp_c") / (B + pl.col("temp_c")))
                    + (pl.col("humedad") / 100).log()
                ).alias("magnus_gamma"),
        ...
        .with_columns(
            [
                (B * pl.col("magnus_gamma") / (A - pl.col("magnus_gamma"))).alias(
                    "punto_rocio"
                ),
                (pl.col("vel_viento_ms") / 0.836)
                .pow(2 / 3)
                .round(0)
                .clip(0, 12)
                .cast(pl.Int8)
...
def add_contamintante_features(df: pl.DataFrame) -> pl.DataFrame:
    ...
    exprs = [(pl.col("pm10") + pl.col("pm25")).alias("pm_total")]

    for contam, rules in breakpoints.items():
        if contam in df.columns:
            exprs.append(
                categorize_contaminante(pl.col(contam), rules).alias(
                    f"{contam}_ica_cat"
                )
            )
...
def write_csv_to_minio(
    ...
    _, year, month, filename = object_key.split("/")
    output_key = f"tipo_parametro={TIPO_PARAMETROS}/year={year}/month={month}/{filename.replace('.xlsx', '.csv')}"
...
def load_config():
    ...
    try:
        ...
        logger.error(
            f"Archivo de configuración '{CONFIG_FILE}' no encontrado. Asegúrate de crearlo."
        )
        raise
    except json.JSONDecodeError:
        logger.error(
            f"Error al decodificar el archivo '{CONFIG_FILE}'. Verifica el formato JSON."
        )
        raise
...
def main():
    ...
    write_csv_to_minio(df, BUCKET_DESTINO, OBJECT_KEY)
